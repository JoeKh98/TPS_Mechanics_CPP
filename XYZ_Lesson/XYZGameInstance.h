...
UCLASS()
class XYZ_LESSON_API UXYZGameInstance : public UGameInstance
{
	GENERATED_BODY()
...
protected: 
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly) //name of the map that will be loaded during the start of online session
		FName LobbyMapName; 
...
protected:
	...
	UPROPERTY(Replicated, EditAnywhere, Category = "Server settings")
		FName ServerName;  //Server name that hosts name

	UPROPERTY(Replicated, EditAnywhere, Category = "Server settings")
		uint32 MaxPlayers; //max num of players that can be in current host game
